...
public class WebShootingSpiderTower : Tower {
  ...
  private void SlowNearbyEnemies(Enemy target) {
    ...
    foreach (var enemy in closestEnemies) {
      ...
      secondaryWebShot.transform.position = ProjectileHandler.GetTargetPosition(target.transform);
  ...
  protected override void TowerUpdate() {
    ...
    if (enemy == null) {
      ...
      mesh.LookAt(ProjectileHandler.GetTargetPosition(enemy.transform));
      firing = true;
    }
